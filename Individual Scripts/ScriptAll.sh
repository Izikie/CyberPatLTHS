...
Users() {
    ...
    if ((${#NeededStandard[@]} == 0)); then
        echo "STANDARD USERS NOT DEFINED"
    fi
    if ((${#NeededUsers[@]} == 0)); then
    ...
    for ((i = 0; i < ${#AdminDiffs[@]}; i++)); do
        if [[ ${NeededUsers[*]} =~ ${AdminDiffs[i]} ]]; then          #if user is on README but not currently admin
            if [[ ${CurrentNormUsers[*]} =~ ${AdminDiffs[i]} ]]; then # user is a standard user that needs to be upgraded
                usermod -aG sudo ${AdminDiffs[i]}
                usermod -aG adm ${AdminDiffs[i]}
                echo "change standard user ${AdminDiffs[i]} to admin" >>UserChangeLog
            elif [[ ! ${CurrentNormUsers[*]} =~ ${AdminDiffs[i]} ]]; then # user is not present on system
                useradd -s /bin/bash -m -G sudo ${AdminDiffs[i]}
                echo "add admin user ${AdminDiffs[i]}" >>UserChangeLog
            fi
        elif [[ ! ${NeededUsers[*]} =~ ${AdminDiffs[i]} ]]; then #user is on system but not readme
            if [[ ${NeededStandard[*]} =~ ${AdminDiffs[i]} ]]; then
    ...
    for ((i = 0; i < ${#StandardDiffs[@]}; i++)); do
        if [[ ${NeededStandard[*]} =~ ${StandardDiffs[i]} ]]; then        #if user is on README but not currently on system
            if [[ ${CurrentAdminUsers[*]} =~ ${StandardDiffs[i]} ]]; then #user is admin that needs to be downgraded(shouldnt happen but is here just in case)
                deluser ${StandardDiffs[i]} sudo
                deluser ${StandardDiffs[i]} adm
                echo "change admin ${StandardDiffs[i]} to standard user" >>UserChangeLog
            elif [[ ! ${CurrentAdminUsers[*]} =~ ${StandardDiffs[i]} ]]; then # user is not present on system
                useradd -s /bin/bash -m ${StandardDiffs[i]}
                echo "add standard user ${StandardDiffs[i]}" >>UserChangeLog
            fi
        elif [[ ! ${NeededStandard[*]} =~ ${StandardDiffs[i]} ]]; then #user is on system but not Readme
            if [[ ${NeededUsers[*]} =~ ${StandardDiffs[i]} ]]; then    #(again somewhat redundant) user is supposed to be admin
    ...
    #========================================================================================================================
    for ((i = 1; i < ${#AllCurrentUsers[@]}; i++)); do
        if [[ ${AllCurrentUsers[i]} != "root" ]]; then
...
UBUpdates() {
    ...
    for ((i = 0; i < ${#debType[@]}; i++)); do
        if grep -iq "${debType[i]} http://archive.canonical.com/ubuntu focal" /etc/apt/sources.list; then
    ...
    for ((i = 0; i < ${#configType[@]}; i++)); do
        if grep -iq "APT::Periodic::${configType[i]}" /etc/apt/apt.conf.d/*auto-upgrades /etc/apt/apt.conf.d/*periodic; then
            sed -i "/APT::Periodic::${configType[i]}/c\APT::Periodic::${configType[i]};" /etc/apt/apt.conf.d/*auto-upgrades /etc/apt/apt.conf.d/*periodic
        else
            echo "APT::Periodic::${configType[i]};" /etc/apt/apt.conf.d/*periodic >>/etc/apt/apt.conf.d/*auto-upgrades
        fi
    done
    for ((i = 0; i < ${#configType[@]}; i++)); do
        if grep -iq "APT::Periodic::AutocleanInterval" /etc/apt/apt.conf.d/*auto-upgrades /etc/apt/apt.conf.d/*periodic; then
            ...
            echo 'APT::Periodic::AutocleanInterval "14";' /etc/apt/apt.conf.d/*periodic >>/etc/apt/apt.conf.d/*auto-upgrades
        fi
    done
...
DEBUpdates() {
    ...
    for ((i = 0; i < ${#debType[@]}; i++)); do
        ...
        if grep -iq "${debType[i]} http://deb.debian.org/debian/ bullseye-updates " /etc/apt/sources.list; then
        ...
        if grep -iq "${debType[i]} http://security.debian.org/debian-security bullseye-security " /etc/apt/sources.list; then
    ...
    for ((i = 0; i < ${#configType[@]}; i++)); do
        if grep -iq "APT::Periodic::${configType[i]}" /etc/apt/apt.conf.d/*unattended-upgrades; then
